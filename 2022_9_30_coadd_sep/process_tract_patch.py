...
OUTPUT_BUTLER = '/global/cscratch1/sd/jjeffers/process_tract_output'
#os.path.expandvars(os.path.join("$SCRATCH", "butler_coadd_sep"))
OUTPUT_DIR = (
    "/global/cscratch1/sd/jjeffers/process_tract_output/" 
    "ssi-cosmodc2/2020_08_02_coadd_sep/ssi_cats"
)

#"/global/homes/b/beckermr/desc_project/groups/fake-source-injection/DC2/"

def flux2mag(f, zp):
    f0 = 10.0**(zp/2.5)
    b = 1e-20
    return -2.5/np.log(10) * (np.arcsinh((f/f0)/(2*b)) + np.log(b))

def _run_sep_and_add_radec(ti, pi, img, zp, err=None, minerr=None):
    if err is None:
    ...
    wcs = ti.getWcs()
    xmin = pi.getOuterBBox().beginX
    ymin = pi.getOuterBBox().beginY
    cat["ra"], cat["dec"] = wcs.pixelToSkyArray(
        cat["x"]+xmin, cat["y"]+ymin,
        degrees=True,
    )
    cat["mag_auto"] = flux2mag(cat["flux_auto"], zp)
    return cat, seg
...
--output /global/cscratch1/sd/jjeffers/process_tract_output/ \
--id tract=%d patch=%s filter=r \
-c fakeType=%s \
--clobber-config --no-versions
""" % (tract, "%d,%d" % pi.getIndex(), ssi_src_file)
    ...
    output_butler._setDefaultMapper
    ...
    orig_det_cat, orig_det_seg = _run_sep_and_add_radec(ti, pi, image, zp)
    ssi_det_cat, ssi_det_seg = _run_sep_and_add_radec(ti, pi, fake_image, zp)
    var_msk = np.isfinite(fake_image.variance.array)
    ssi_truth_cat, ssi_truth_seg = _run_sep_and_add_radec(
        ti,
        pi,
        (fake_image.image.array - image.image.array).copy(),
        zp,
        err=np.zeros_like(np.sqrt(fake_image.variance.array.copy())),
        minerr=np.mean(np.sqrt(fake_image.variance.array.copy()[var_msk])),
    )
