...
def _run_func(tract, patch, seed):
    logfile = "log_files/log_tract%d_patch%d_seed%d.oe" % (
        tract, patch, seed
    )
    subprocess.run(
...
def main(tract, seed):
    ...
    subprocess.run("mkdir -p log_files", shell=True, check=True)

    with joblib.Parallel(backend="loky", n_jobs=16, verbose=100) as par:
        par(jobs)
